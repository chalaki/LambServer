...
# work with minikube
    minikube start  # stats 'minikube' VM  on virtualbox
    minikube start -p shop # stats 'shop' VM  on virtualbox
    minikube config set profile shop # all future minikube cmds now refer to 'shop' profile which uses 'shop' VM
    minikube docker-env # follow instruction to run last line to have docker client point to minikube docker host  
    minikube dashboard  
    minikube service node  --url # returns the url
    minikube service node        # launches the url
